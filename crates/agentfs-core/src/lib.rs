...
pub enum AgentFsError {
    ...
    InvalidPath,
    #[error("unknown AgentFS operation: {0}")]
...
pub fn normalize_path(
...
) -> Result<NormalizedPath, AgentFsError> {
    if path.contains('\0') {
    ...
    if absolute && !path.starts_with('/') {
        return Err(AgentFsError::AbsolutePathRequired);
    }
    if !absolute && path.starts_with('/') {
        return Err(AgentFsError::RelativePathRequired);
    }
        ...
        match segment {
            ...
            ".." => {
                if segments.pop().is_none() {
                    return Err(AgentFsError::PathEscape);
                }
            }
...
pub fn classify_risk(operation: &str) -> Result<RiskLevel, AgentFsError> {
    let level = match operation {
        ...
        other => return Err(AgentFsError::UnknownOperation(other.to_string())),
    };
    Ok(level)
}
...
mod tests {
    ...
    fn absolute_mode_requires_absolute_input() {
        assert_eq!(
            ...
            Err(AgentFsError::AbsolutePathRequired)
        );
    }
    ...
    fn unknown_operation_fails_closed() {
        assert_eq!(
            ...
            Err(AgentFsError::UnknownOperation("shell-anything".into()))
        );
    }
}
